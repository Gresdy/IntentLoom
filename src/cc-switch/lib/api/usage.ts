...
import type { UsageResult } from "@cc-switch/types";
import type { AppId } from "./types";
import type { TemplateType } from "@cc-switch/config/constants";
...
export const usageApi = {
  ...
  getModelPricing: async (): Promise<ModelPricing[]> => {
    return invoke("cc_switch_get_model_pricing");
  },
  ...
  syncSessionUsage: async (): Promise<SessionSyncResult> => {
    return invoke("cc_switch_sync_session_usage");
  },

  getDataSourceBreakdown: async (): Promise<DataSourceSummary[]> => {
    return invoke("cc_switch_get_usage_data_sources");
  },
};
