...
export const proxyApi = {
  ...
  async startProxyServer(): Promise<ProxyServerInfo> {
    return invoke("cc_switch_start_proxy_server");
  },

  // 停止代理服务器并恢复配置
  async stopProxyWithRestore(): Promise<void> {
    return invoke("cc_switch_stop_proxy_with_restore");
  },

  // 获取代理服务器状态
  async getProxyStatus(): Promise<ProxyStatus> {
    return invoke("cc_switch_get_proxy_status");
  },

  // 检查代理服务器是否正在运行
  async isProxyRunning(): Promise<boolean> {
    return invoke("cc_switch_is_proxy_running");
  },

  // 检查是否处于接管模式
  async isLiveTakeoverActive(): Promise<boolean> {
    return invoke("cc_switch_is_live_takeover_active");
  },
  ...
  async getProxyTakeoverStatus(): Promise<ProxyTakeoverStatus> {
    return invoke("cc_switch_get_proxy_takeover_status");
  },
  ...
  async getProxyConfig(): Promise<ProxyConfig> {
    return invoke("cc_switch_get_proxy_config");
  },
  ...
  async getGlobalProxyConfig(): Promise<GlobalProxyConfig> {
    return invoke("cc_switch_get_global_proxy_config");
  },
