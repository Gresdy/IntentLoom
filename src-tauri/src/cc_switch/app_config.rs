...
use crate::cc_switch::services::skill::SkillStore;
...
pub struct PromptConfig {
    #[serde(default)]
    pub prompts: HashMap<String, crate::cc_switch::prompt::Prompt>,
}
...
impl MultiAppConfig {
    ...
    fn auto_import_prompt_if_exists(config: &mut Self, app: AppType) -> Result<bool, AppError> {
        ...
        let prompt = crate::cc_switch::prompt::Prompt {
            id: id.clone(),
            name: format!(
...
mod tests {
    ...
    fn write_prompt_file(app: AppType, content: &str) {
        let path = crate::cc_switch::prompt_files::prompt_file_path(&app).expect("prompt path");
