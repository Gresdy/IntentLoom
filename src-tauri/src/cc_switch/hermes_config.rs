...
use crate::cc_switch::config::{atomic_write, get_app_config_dir};
use crate::cc_switch::error::AppError;
use crate::cc_switch::settings::{effective_backup_retain_count, get_hermes_override_dir};
use chrono::Local;
...
fn default_hermes_dir() -> PathBuf {
    windows_local_hermes_dir(
        ...
        &crate::cc_switch::config::get_home_dir(),
    )
}
...
fn default_hermes_dir() -> PathBuf {
    crate::cc_switch::config::get_home_dir().join(".hermes")
}
...
";
    ...
    fn models_array_to_dict_strips_id_and_preserves_order() {
        ...
        let mut sorted_keys: Vec<String> = keys.into_iter().cloned().collect(); sorted_keys.sort(); assert_eq!(sorted_keys, vec!["bar".to_string(), "baz".to_string(), "foo".to_string()]);
...
";
    ...
    fn hermes_home_env_takes_precedence_over_platform_default() {
        with_test_home(|| {
            ...
            let mut s = crate::cc_switch::settings::get_settings();
            s.hermes_config_dir = None;
            crate::cc_switch::settings::update_settings(s).unwrap();
