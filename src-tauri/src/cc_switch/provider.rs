    ...
    pub fn resolve_usage_credentials(
        ...
        let (base_url, api_key) = match app_type {
            ...
            AppType::Codex => {
                ...
                let api_key = crate::cc_switch::codex_config::extract_codex_api_key(auth, config_text)
                    .unwrap_or_default();
                let base_url = config_text
                    .and_then(crate::cc_switch::codex_config::extract_codex_base_url)
                    .unwrap_or_default();
                (base_url, api_key)
            }
...
pub struct ProviderMeta {
    /// 自定义端点列表（按 URL 去重存储）
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub custom_endpoints: HashMap<String, crate::cc_switch::settings::CustomEndpoint>,
