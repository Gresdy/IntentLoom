...
const H_TIER_NAMES: &[&str] = &[crate::cc_switch::services::subscription::TIER_FIVE_HOUR];
const W_TIER_NAMES: &[&str] = &[
    crate::cc_switch::services::subscription::TIER_WEEKLY_LIMIT,
    crate::cc_switch::services::subscription::TIER_SEVEN_DAY,
    crate::cc_switch::services::subscription::TIER_SEVEN_DAY_OPUS,
    crate::cc_switch::services::subscription::TIER_SEVEN_DAY_SONNET,
];
// 火山方舟 Agent/Coding Plan 的月窗口（5h/周/月 三档）。
const M_TIER_NAMES: &[&str] = &[crate::cc_switch::services::subscription::TIER_MONTHLY];
const GEMINI_PRO_TIER_NAMES: &[&str] = &[crate::cc_switch::services::subscription::TIER_GEMINI_PRO];
const GEMINI_FLASH_TIER_NAMES: &[&str] = &[crate::cc_switch::services::subscription::TIER_GEMINI_FLASH];
const GEMINI_FLASH_LITE_TIER_NAMES: &[&str] =
    &[crate::cc_switch::services::subscription::TIER_GEMINI_FLASH_LITE];
...
fn format_subscription_summary(
    quota: &crate::cc_switch::services::subscription::SubscriptionQuota,
) -> Option<String> {
    if !quota.success {
        return None;
    }
...
fn tier_pct(data: &crate::cc_switch::provider::UsageData) -> Option<f64> {
...
fn format_script_summary(result: &crate::cc_switch::provider::UsageResult) -> Option<String> {
    if !result.success {
        return None;
    }
...
fn provider_uses_official_subscription(provider: &crate::cc_switch::provider::Provider) -> bool {
    provider
        .meta
        .as_ref()
...
fn format_usage_suffix(
    app_state: &AppState,
    app_type: &AppType,
    provider: &crate::cc_switch::provider::Provider,
    provider_id: &str,
) -> Option<String> {
...
fn sort_providers(
    providers: &indexmap::IndexMap<String, crate::cc_switch::provider::Provider>,
) -> Vec<(&String, &crate::cc_switch::provider::Provider)> {
...
fn handle_auto_click(app: &tauri::AppHandle, app_type: &AppType) -> Result<(), AppError> {
    if let Some(app_state) = app.try_state::<AppState>() {
        ...
        if queue.is_empty() {
            let current_id =
                crate::cc_switch::settings::get_effective_current_provider(&app_state.db, app_type)?;
...
fn handle_provider_click(
    ...
    if let Some(app_state) = app.try_state::<AppState>() {
        ...
        // 由用户在页面/设置中手动开启。
        crate::cc_switch::services::ProviderService::switch(app_state.inner(), app_type.clone(), provider_id)?;
...
pub fn create_tray_menu(
    ...
    let app_settings = crate::cc_switch::settings::get_settings();
    ...
    for section in TRAY_SECTIONS.iter() {
        ...
        let current_id =
            crate::cc_switch::settings::get_effective_current_provider(&app_state.db, &section.app_type)?
                .unwrap_or_default();
    ...
    match event_id {
        "show_main" => {
            if let Some(window) = app.get_webview_window("main") {
                ...
                #[cfg(target_os = "linux")]
                {
                    crate::cc_switch::linux_fix::nudge_main_window(window.clone());
                }
                #[cfg(target_os = "macos")]
                {
                    apply_tray_policy(app, true);
                }
            } else if crate::cc_switch::lightweight::is_lightweight_mode() {
                if let Err(e) = crate::cc_switch::lightweight::exit_lightweight_mode(app) {
        ...
        "lightweight_mode" => {
            if crate::cc_switch::lightweight::is_lightweight_mode() {
                if let Err(e) = crate::cc_switch::lightweight::exit_lightweight_mode(app) {
                    log::error!("退出轻量模式失败: {e}");
                }
            } else if let Err(e) = crate::cc_switch::lightweight::enter_lightweight_mode(app) {
...
pub(crate) async fn refresh_all_usage_in_tray(app: &tauri::AppHandle) {
    use crate::cc_switch::commands::CopilotAuthState;
    ...
    let visible_apps = crate::cc_switch::settings::get_settings()
        .visible_apps
        .unwrap_or_default();
    ...
    for section in TRAY_SECTIONS.iter() {
        ...
        let current_id =
            match crate::cc_switch::settings::get_effective_current_provider(&app_state.db, &section.app_type)
            {
                Ok(Some(id)) => id,
                Ok(None) => continue,
                Err(e) => {
            ...
            && (!is_official_provider || provider_uses_official_subscription(&current))
        {
            ...
            let _copilot_state = app.state::<CopilotAuthState>();
            let provider_id = current_id.clone();
            let app_str = app_type_str.to_string();
            script_futures.push(async move {
                if let Err(e) = crate::cc_switch::commands::queryProviderUsage(
                    provider_id.clone(),
                    app_str,
                )
                {
...
mod tests {
    ...
    use crate::cc_switch::provider::{UsageData, UsageResult};
    use crate::cc_switch::services::subscription::{
...
/// Stub: cc-switch 在设置/主题命令里调用的窗口主题切换桩。
/// IntentLoom 用 tauri-plugin-window-state 与 ThemeProvider 处理主题，
/// cc_switch 的具体实现留空即可。
#[allow(dead_code)]
pub fn set_window_theme(_app: &tauri::AppHandle, _theme: &str) {}
