...
use crate::cc_switch::provider::{ClaudeDesktopMode, Provider, ProviderMeta, UsageScript};
...
fn merge_codex_config(
    ...
    if request.api_key.as_ref().is_none_or(|s| s.is_empty()) {
        ...
        if let Some(api_key) =
            crate::cc_switch::codex_config::extract_codex_api_key(config.get("auth"), config_str)
        {
