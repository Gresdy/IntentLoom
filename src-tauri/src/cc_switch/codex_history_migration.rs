...
use crate::cc_switch::config::{atomic_write, copy_file, get_app_config_dir};
use crate::cc_switch::database::{is_official_seed_id, Database};
...
pub fn maybe_migrate_codex_third_party_history_provider_bucket(
    ...
    if crate::cc_switch::settings::is_codex_third_party_history_provider_bucket_migrated() {
    ...
    if source_provider_ids.is_empty() {
        crate::cc_switch::settings::mark_codex_third_party_history_provider_bucket_migrated(
            CodexThirdPartyHistoryProviderBucketMigration {
...
pub fn maybe_migrate_codex_provider_template_bucket(
    ...
    if crate::cc_switch::settings::is_codex_provider_template_migrated() {
    ...
    crate::cc_switch::settings::mark_codex_provider_template_migrated(CodexProviderTemplateMigration {
        completed_at: Utc::now().to_rfc3339(),
...
pub fn maybe_migrate_codex_official_history_to_unified_bucket(
    ...
    if !crate::cc_switch::settings::unify_codex_session_history() {
        return Ok(CodexHistoryProviderBucketMigrationOutcome {
            skipped_reason: Some("unify_toggle_off".to_string()),
            ..Default::default()
        });
    }
    if !crate::cc_switch::settings::unify_codex_migrate_existing_requested() {
        return Ok(CodexHistoryProviderBucketMigrationOutcome {
    ...
    if crate::cc_switch::settings::is_codex_official_history_unify_migrated_for_dir(&codex_dir_key) {
        return Ok(CodexHistoryProviderBucketMigrationOutcome {
            skipped_reason: Some("already_migrated".to_string()),
            ..Default::default()
        });
    ...
    let marker_written = crate::cc_switch::settings::mark_codex_official_history_unify_migrated_if_enabled(
...
fn resolve_user_path(raw: &str) -> PathBuf {
    if raw == "~" {
        return crate::cc_switch::config::get_home_dir();
    }
    if let Some(rest) = raw.strip_prefix("~/") {
        return crate::cc_switch::config::get_home_dir().join(rest);
    }
    if let Some(rest) = raw.strip_prefix("~\\") {
        return crate::cc_switch::config::get_home_dir().join(rest);
    }
