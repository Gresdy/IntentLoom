...
use crate::cc_switch::init_status::{InitErrorPayload, SkillsMigrationPayload};
use crate::cc_switch::services::ProviderService;
...
/// 打开外部链接
#[tauri::command]
pub async fn cc_switch_open_external(app: AppHandle, url: String) -> Result<bool, String> {
...
#[tauri::command]
pub async fn cc_switch_copy_text_to_clipboard(text: String) -> Result<bool, String> {
...
/// 检查更新
#[tauri::command]
pub async fn cc_switch_check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
...
/// 判断是否为便携版（绿色版）运行
#[tauri::command]
pub async fn cc_switch_is_portable_mode() -> Result<bool, String> {
...
#[tauri::command]
pub async fn cc_switch_get_init_error() -> Result<Option<InitErrorPayload>, String> {
    Ok(crate::cc_switch::init_status::get_init_error())
}

/// 获取 JSON→SQLite 迁移结果（若有）。
/// 只返回一次 true，之后返回 false，用于前端显示一次性 Toast 通知。
#[tauri::command]
pub async fn cc_switch_get_migration_result() -> Result<bool, String> {
    Ok(crate::cc_switch::init_status::take_migration_success())
}
...
#[tauri::command]
pub async fn cc_switch_get_tool_versions(
...
#[tauri::command]
pub async fn cc_switch_run_tool_lifecycle_action(
    tools: Vec<String>,
    action: String,
    wsl_shell_by_tool: Option<HashMap<String, WslShellPreferenceInput>>,
) -> Result<(), String> {
...
/// 散落多套下游判定。
#[tauri::command]
pub async fn cc_switch_probe_tool_installations(
    tools: Vec<String>,
...
fn wsl_distro_for_tool(tool: &str) -> Option<String> {
    let override_dir = match tool {
        "claude" => crate::cc_switch::settings::get_claude_override_dir(),
        "codex" => crate::cc_switch::settings::get_codex_override_dir(),
        "gemini" => crate::cc_switch::settings::get_gemini_override_dir(),
        "opencode" => crate::cc_switch::settings::get_opencode_override_dir(),
        "openclaw" => crate::cc_switch::settings::get_openclaw_override_dir(),
        "hermes" => crate::cc_switch::settings::get_hermes_override_dir(),
        _ => None,
    }?;
...
#[allow(non_snake_case)]
#[tauri::command]
pub async fn cc_switch_open_provider_terminal(
    state: State<'_, crate::cc_switch::store::AppState>,
    app: String,
...
fn launch_macos_terminal(config_file: &std::path::Path, cwd: Option<&Path>) -> Result<(), String> {
    ...
    let preferred = crate::cc_switch::settings::get_preferred_terminal();
