...
use crate::cc_switch::proxy::providers::copilot_auth::{
...
#[tauri::command]
pub async fn cc_switch_copilot_start_device_flow(
...
pub async fn copilot_poll_for_auth(
        ...
        .await
    {
        ...
        Ok(None) => Ok(false),
        Err(crate::cc_switch::proxy::providers::copilot_auth::CopilotAuthError::AuthorizationPending) => {
            Ok(false)
        }
        Err(e) => {
...
/// 列出所有已认证的账号
#[tauri::command]
pub async fn cc_switch_copilot_list_accounts(
    state: State<'_, CopilotAuthState>,
...
/// 移除指定账号
#[tauri::command(rename_all = "camelCase")]
pub async fn cc_switch_copilot_remove_account(
    account_id: String,
    state: State<'_, CopilotAuthState>,
) -> Result<(), String> {
    let auth_manager = state.0.write().await;
...
/// 获取认证状态（包含所有账号）
#[tauri::command]
pub async fn cc_switch_copilot_get_auth_status(
    state: State<'_, CopilotAuthState>,
...
/// 检查是否已认证（有任意账号）
#[tauri::command]
pub async fn cc_switch_copilot_is_authenticated(state: State<'_, CopilotAuthState>) -> Result<bool, String> {
    let auth_manager = state.0.read().await;
...
/// 注销所有 Copilot 认证
#[tauri::command]
pub async fn cc_switch_copilot_logout(state: State<'_, CopilotAuthState>) -> Result<(), String> {
    let auth_manager = state.0.write().await;
...
/// 内部使用，用于代理请求
#[tauri::command]
pub async fn cc_switch_copilot_get_token(state: State<'_, CopilotAuthState>) -> Result<String, String> {
    let auth_manager = state.0.read().await;
    auth_manager
        .get_valid_token()
        .await
...
#[tauri::command]
pub async fn cc_switch_copilot_get_models(
    state: State<'_, CopilotAuthState>,
...
#[tauri::command(rename_all = "camelCase")]
pub async fn cc_switch_copilot_get_models_for_account(
    account_id: String,
    state: State<'_, CopilotAuthState>,
) -> Result<Vec<CopilotModel>, String> {
    let auth_manager = state.0.read().await;
...
#[tauri::command]
pub async fn cc_switch_copilot_get_usage(
    state: State<'_, CopilotAuthState>,
...
#[tauri::command(rename_all = "camelCase")]
pub async fn cc_switch_copilot_get_usage_for_account(
    account_id: String,
    state: State<'_, CopilotAuthState>,
) -> Result<CopilotUsageResponse, String> {
    let auth_manager = state.0.read().await;
