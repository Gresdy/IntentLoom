...
use crate::cc_switch::claude_mcp;
use crate::cc_switch::services::McpService;
use crate::cc_switch::store::AppState;

/// 获取 Claude MCP 状态
#[tauri::command]
pub async fn cc_switch_get_claude_mcp_status() -> Result<claude_mcp::McpStatus, String> {
    claude_mcp::get_mcp_status().map_err(|e| e.to_string())
}

/// 读取 mcp.json 文本内容
#[tauri::command]
pub async fn cc_switch_read_claude_mcp_config() -> Result<Option<String>, String> {
    claude_mcp::read_mcp_json().map_err(|e| e.to_string())
}

/// 新增或更新一个 MCP 服务器条目
#[tauri::command]
pub async fn cc_switch_upsert_claude_mcp_server(id: String, spec: serde_json::Value) -> Result<bool, String> {
    claude_mcp::upsert_mcp_server(&id, spec).map_err(|e| e.to_string())
}

/// 删除一个 MCP 服务器条目
#[tauri::command]
pub async fn cc_switch_delete_claude_mcp_server(id: String) -> Result<bool, String> {
    claude_mcp::delete_mcp_server(&id).map_err(|e| e.to_string())
}

/// 校验命令是否在 PATH 中可用（不执行）
#[tauri::command]
pub async fn cc_switch_validate_mcp_command(cmd: String) -> Result<bool, String> {
...
pub async fn cc_switch_get_mcp_config(
    state: State<'_, AppState>,
    app: String,
) -> Result<McpConfigResponse, String> {
    let config_path = crate::cc_switch::config::get_app_config_path()
        .to_string_lossy()
        .to_string();
...
#[tauri::command]
pub async fn cc_switch_upsert_mcp_server_in_config(
    state: State<'_, AppState>,
    app: String,
    id: String,
    ...
    use crate::cc_switch::app_config::McpServer;
    ...
    let mut new_server = if let Some(mut existing) = existing_server {
        ...
        // 创建新服务器
        let mut apps = crate::cc_switch::app_config::McpApps::default();
...
#[tauri::command]
pub async fn cc_switch_delete_mcp_server_in_config(
    state: State<'_, AppState>,
...
#[tauri::command]
#[allow(deprecated)] // 兼容层命令，内部调用已废弃的 Service 方法
pub async fn cc_switch_set_mcp_enabled(
    state: State<'_, AppState>,
    app: String,
    id: String,
    enabled: bool,
...
/// 获取所有 MCP 服务器（统一结构）
#[tauri::command]
pub async fn cc_switch_get_mcp_servers(
    state: State<'_, AppState>,
...
/// 添加或更新 MCP 服务器
#[tauri::command]
pub async fn cc_switch_upsert_mcp_server(
    state: State<'_, AppState>,
    server: McpServer,
) -> Result<(), String> {
...
/// 删除 MCP 服务器
#[tauri::command]
pub async fn cc_switch_delete_mcp_server(state: State<'_, AppState>, id: String) -> Result<bool, String> {
    McpService::delete_server(&state, &id).map_err(|e| e.to_string())
}

/// 切换 MCP 服务器在指定应用的启用状态
#[tauri::command]
pub async fn cc_switch_toggle_mcp_app(
    state: State<'_, AppState>,
    server_id: String,
    app: String,
    enabled: bool,
...
#[tauri::command]
pub async fn cc_switch_import_mcp_from_apps(state: State<'_, AppState>) -> Result<usize, String> {
    let mut total = 0;
