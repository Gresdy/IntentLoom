...
use crate::cc_switch::database::backup::BackupEntry;
use crate::cc_switch::database::Database;
use crate::cc_switch::error::AppError;
use crate::cc_switch::services::provider::ProviderService;
...
/// 导出数据库为 SQL 备份
#[tauri::command]
pub async fn cc_switch_export_config_to_file(
...
/// 从 SQL 备份导入数据库
#[tauri::command]
pub async fn cc_switch_import_config_from_file(
    #[allow(non_snake_case)] filePath: String,
    state: State<'_, AppState>,
) -> Result<Value, String> {
    let db = state.db.clone();
...
#[tauri::command]
pub async fn cc_switch_sync_current_providers_live(state: State<'_, AppState>) -> Result<Value, String> {
    let db = state.db.clone();
    tauri::async_runtime::spawn_blocking(move || {
...
#[tauri::command]
pub async fn cc_switch_create_db_backup(state: State<'_, AppState>) -> Result<String, String> {
    let db = state.db.clone();
...
#[tauri::command]
pub fn cc_switch_list_db_backups() -> Result<Vec<BackupEntry>, String> {
    Database::list_backups().map_err(|e| e.to_string())
}

/// Restore database from a backup file
#[tauri::command]
pub async fn cc_switch_restore_db_backup(
    state: State<'_, AppState>,
    filename: String,
...
#[tauri::command]
pub fn cc_switch_rename_db_backup(
...
#[tauri::command]
pub fn cc_switch_delete_db_backup(filename: String) -> Result<(), String> {
