...
use crate::cc_switch::commands::copilot::CopilotAuthState;
use crate::cc_switch::error::AppError;
use crate::cc_switch::provider::{ClaudeDesktopMode, Provider};
use crate::cc_switch::services::{
...
/// 获取所有供应商
#[tauri::command]
pub fn cc_switch_get_providers(
...
#[tauri::command]
pub fn cc_switch_get_current_provider(state: State<'_, AppState>, app: String) -> Result<String, String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    ProviderService::current(state.inner(), app_type).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn cc_switch_add_provider(
    state: State<'_, AppState>,
    app: String,
    provider: Provider,
...
#[tauri::command]
pub fn cc_switch_update_provider(
    state: State<'_, AppState>,
    app: String,
    provider: Provider,
...
#[tauri::command]
pub fn cc_switch_delete_provider(
    state: State<'_, AppState>,
    app: String,
    id: String,
) -> Result<bool, String> {
...
#[tauri::command]
pub fn cc_switch_remove_provider_from_live_config(
...
#[tauri::command]
pub fn cc_switch_switch_provider(
    state: State<'_, AppState>,
    app: String,
    id: String,
...
#[tauri::command]
pub fn cc_switch_import_default_config(state: State<'_, AppState>, app: String) -> Result<bool, String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    import_default_config_internal(&state, app_type).map_err(Into::into)
}

#[tauri::command]
pub async fn cc_switch_get_claude_desktop_status(
    state: State<'_, AppState>,
) -> Result<crate::cc_switch::claude_desktop_config::ClaudeDesktopStatus, String> {
    let proxy_running = state.proxy_service.is_running().await;
    crate::cc_switch::claude_desktop_config::get_status(state.db.as_ref(), proxy_running)
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn cc_switch_get_claude_desktop_default_routes(
) -> Vec<crate::cc_switch::claude_desktop_config::ClaudeDesktopDefaultRoute> {
    crate::cc_switch::claude_desktop_config::default_proxy_routes()
}

#[tauri::command]
pub fn cc_switch_import_claude_desktop_providers_from_claude(
    state: State<'_, AppState>,
) -> Result<usize, String> {
    ...
    for provider in claude_providers.values() {
        ...
        if crate::cc_switch::claude_desktop_config::is_compatible_direct_provider(provider)
    ...
    if let Err(e) = state.db.ensure_official_seed_by_id(
        crate::cc_switch::database::CLAUDE_DESKTOP_OFFICIAL_PROVIDER_ID,
...
#[tauri::command]
pub fn cc_switch_ensure_claude_desktop_official_provider(state: State<'_, AppState>) -> Result<bool, String> {
    state
        .db
...
fn claude_provider_models_are_claude_safe(provider: &Provider) -> bool {
    ...
    .all(crate::cc_switch::claude_desktop_config::is_claude_safe_model_id)
}

pub(crate) fn suggested_claude_desktop_routes(
    provider: &Provider,
) -> Option<std::collections::HashMap<String, crate::cc_switch::provider::ClaudeDesktopModelRoute>> {
    let env = provider
        .settings_config
        .get("env")
        .and_then(|value| value.as_object())?;
    ...
    fn add_route(
        routes: &mut std::collections::HashMap<String, crate::cc_switch::provider::ClaudeDesktopModelRoute>,
        ...
        let marker = crate::cc_switch::claude_desktop_config::ONE_M_CONTEXT_MARKER.as_bytes();
        ...
        let label_override = explicit_label_override.clone().or_else(|| {
            (!crate::cc_switch::claude_desktop_config::is_claude_safe_model_id(stripped_model))
        ...
        let merge_into = |existing: &mut crate::cc_switch::provider::ClaudeDesktopModelRoute| {
        ...
        routes
            ...
            .or_insert_with(|| crate::cc_switch::provider::ClaudeDesktopModelRoute {
    ...
    for spec in crate::cc_switch::claude_desktop_config::DEFAULT_PROXY_ROUTES {
        add_route(
            &mut routes,
            env,
            spec.route_id,
    ...
    if routes.is_empty() {
        let primary_route = crate::cc_switch::claude_desktop_config::DEFAULT_PROXY_ROUTES[0].route_id;
        add_route(
            &mut routes,
            env,
            primary_route,
...
#[tauri::command]
pub async fn cc_switch_queryProviderUsage(
    app_handle: tauri::AppHandle,
    state: State<'_, AppState>,
    copilot_state: State<'_, CopilotAuthState>,
    #[allow(non_snake_case)] providerId: String, // 使用 camelCase 匹配前端
    app: String,
) -> Result<crate::cc_switch::provider::UsageResult, String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    // inner 可能以两种形式失败：
    ...
    let snapshot = match &inner {
        Ok(r) => r.clone(),
        Err(err_msg) => crate::cc_switch::provider::UsageResult {
            success: false,
            data: None,
    ...
    crate::cc_switch::tray::schedule_tray_refresh(&app_handle);
    inner
}
...
fn resolve_coding_plan_credentials(
    ...
    usage_script: Option<&crate::cc_switch::provider::UsageScript>,
) -> (String, String) {
...
async fn query_provider_usage_inner(
    ...
    if template_type == TEMPLATE_TYPE_GITHUB_COPILOT {
        ...
        return Ok(crate::cc_switch::provider::UsageResult {
            success: true,
            data: Some(vec![crate::cc_switch::provider::UsageData {
    ...
    if template_type == TEMPLATE_TYPE_TOKEN_PLAN {
        ...
        let quota = crate::cc_switch::services::coding_plan::get_coding_plan_quota(
            &base_url,
            &api_key,
        ...
        let data: Vec<crate::cc_switch::provider::UsageData> = quota
            .tiers
            .iter()
            .map(|tier| {
                let total = 100.0;
                ...
                crate::cc_switch::provider::UsageData {
    ...
    if template_type == TEMPLATE_TYPE_BALANCE {
        ...
        return crate::cc_switch::services::balance::get_balance(&base_url, &api_key)
            .await
    ...
    if template_type == TEMPLATE_TYPE_OFFICIAL_SUBSCRIPTION {
        ...
        let quota = crate::cc_switch::services::subscription::get_subscription_quota(app_type.as_str())
            .await
        ...
        let data: Vec<crate::cc_switch::provider::UsageData> = quota
            .tiers
            .iter()
            .map(|tier| crate::cc_switch::provider::UsageData {
                plan_name: Some(tier.name.clone()),
...
#[tauri::command]
pub async fn cc_switch_testUsageScript(
    state: State<'_, AppState>,
...
#[tauri::command]
pub fn cc_switch_read_live_provider_settings(app: String) -> Result<serde_json::Value, String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    ProviderService::read_live_settings(app_type).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn cc_switch_test_api_endpoints(
    urls: Vec<String>,
...
#[tauri::command]
pub fn cc_switch_get_custom_endpoints(
    state: State<'_, AppState>,
    app: String,
    #[allow(non_snake_case)] providerId: String,
) -> Result<Vec<crate::cc_switch::settings::CustomEndpoint>, String> {
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn cc_switch_add_custom_endpoint(
    state: State<'_, AppState>,
    app: String,
    #[allow(non_snake_case)] providerId: String,
    url: String,
...
#[tauri::command]
pub fn cc_switch_remove_custom_endpoint(
    state: State<'_, AppState>,
    app: String,
    #[allow(non_snake_case)] providerId: String,
    url: String,
...
#[tauri::command]
pub fn cc_switch_update_endpoint_last_used(
    state: State<'_, AppState>,
    app: String,
    #[allow(non_snake_case)] providerId: String,
    url: String,
...
#[tauri::command]
pub fn cc_switch_update_providers_sort_order(
    state: State<'_, AppState>,
    app: String,
...
use crate::cc_switch::provider::UniversalProvider;
...
#[tauri::command]
pub fn cc_switch_get_universal_providers(
    state: State<'_, AppState>,
...
#[tauri::command]
pub fn cc_switch_get_universal_provider(
    state: State<'_, AppState>,
    id: String,
...
#[tauri::command]
pub fn cc_switch_upsert_universal_provider(
    app: AppHandle,
    state: State<'_, AppState>,
...
#[tauri::command]
pub fn cc_switch_delete_universal_provider(
    app: AppHandle,
    state: State<'_, AppState>,
    id: String,
) -> Result<bool, String> {
...
#[tauri::command]
pub fn cc_switch_sync_universal_provider(
    app: AppHandle,
    state: State<'_, AppState>,
    id: String,
) -> Result<bool, String> {
...
#[tauri::command]
pub fn cc_switch_import_opencode_providers_from_live(state: State<'_, AppState>) -> Result<usize, String> {
    crate::cc_switch::services::provider::import_opencode_providers_from_live(state.inner())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn cc_switch_get_opencode_live_provider_ids() -> Result<Vec<String>, String> {
    crate::cc_switch::opencode_config::get_providers()
...
mod import_claude_desktop_tests {
    ...
    use crate::cc_switch::provider::{Provider, ProviderMeta};
    use serde_json::json;
...
mod native_query_credentials_tests {
    ...
    use crate::cc_switch::app_config::AppType;
    use crate::cc_switch::provider::{Provider, UsageScript};
    use serde_json::json;
