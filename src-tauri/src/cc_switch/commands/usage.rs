...
/// 获取使用量汇总
#[tauri::command]
pub fn cc_switch_get_usage_summary(
...
#[tauri::command]
pub fn cc_switch_get_usage_summary_by_app(
    state: State<'_, AppState>,
    start_date: Option<i64>,
    end_date: Option<i64>,
    provider_name: Option<String>,
...
/// 获取每日趋势
#[tauri::command]
pub fn cc_switch_get_usage_trends(
    state: State<'_, AppState>,
    start_date: Option<i64>,
    end_date: Option<i64>,
    app_type: Option<String>,
...
/// 获取 Provider 统计
#[tauri::command]
pub fn cc_switch_get_provider_stats(
    state: State<'_, AppState>,
    start_date: Option<i64>,
    end_date: Option<i64>,
    app_type: Option<String>,
...
/// 获取模型统计
#[tauri::command]
pub fn cc_switch_get_model_stats(
    state: State<'_, AppState>,
    start_date: Option<i64>,
    end_date: Option<i64>,
    app_type: Option<String>,
...
/// 获取请求日志列表
#[tauri::command]
pub fn cc_switch_get_request_logs(
    state: State<'_, AppState>,
    filters: LogFilters,
    page: u32,
    page_size: u32,
...
/// 获取单个请求详情
#[tauri::command]
pub fn cc_switch_get_request_detail(
    state: State<'_, AppState>,
    request_id: String,
...
/// 获取模型定价列表
#[tauri::command]
pub fn cc_switch_get_model_pricing(state: State<'_, AppState>) -> Result<Vec<ModelPricingInfo>, AppError> {
    log::info!("获取模型定价列表");
...
/// 更新模型定价
#[tauri::command]
pub fn cc_switch_update_model_pricing(
    state: State<'_, AppState>,
    model_id: String,
    display_name: String,
    input_cost: String,
...
/// 检查 Provider 使用限额
#[tauri::command]
pub fn cc_switch_check_provider_limits(
    state: State<'_, AppState>,
    provider_id: String,
    app_type: String,
...
/// 删除模型定价
#[tauri::command]
pub fn cc_switch_delete_model_pricing(state: State<'_, AppState>, model_id: String) -> Result<(), AppError> {
    let db = state.db.clone();
    let conn = crate::cc_switch::database::lock_conn!(db.conn);
...
/// 手动触发会话日志同步
#[tauri::command]
pub fn cc_switch_sync_session_usage(
    state: State<'_, AppState>,
) -> Result<crate::cc_switch::services::session_usage::SessionSyncResult, AppError> {
    // 同步 Claude 会话日志
    let mut result = crate::cc_switch::services::session_usage::sync_claude_session_logs(&state.db)?;

    // 同步 Codex 使用数据
    match crate::cc_switch::services::session_usage_codex::sync_codex_usage(&state.db) {
        Ok(codex_result) => {
    ...
    // 同步 Gemini 使用数据
    match crate::cc_switch::services::session_usage_gemini::sync_gemini_usage(&state.db) {
        Ok(gemini_result) => {
    ...
    // 同步 OpenCode 使用数据
    match crate::cc_switch::services::session_usage_opencode::sync_opencode_usage(&state.db) {
        Ok(opencode_result) => {
...
/// 获取数据来源分布
#[tauri::command]
pub fn cc_switch_get_usage_data_sources(
    state: State<'_, AppState>,
) -> Result<Vec<crate::cc_switch::services::session_usage::DataSourceSummary>, AppError> {
    crate::cc_switch::services::session_usage::get_data_source_breakdown(&state.db)
}
