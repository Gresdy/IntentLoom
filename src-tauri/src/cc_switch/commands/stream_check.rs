...
use crate::cc_switch::services::stream_check::{
...
/// 连通性检查（单个供应商）
#[tauri::command]
pub async fn cc_switch_stream_check_provider(
...
/// 批量连通性检查
#[tauri::command]
pub async fn cc_switch_stream_check_all_providers(
    state: State<'_, AppState>,
    copilot_state: State<'_, CopilotAuthState>,
    app_type: AppType,
...
/// 获取连通性检查配置
#[tauri::command]
pub fn cc_switch_get_stream_check_config(state: State<'_, AppState>) -> Result<StreamCheckConfig, AppError> {
    state.db.get_stream_check_config()
}

/// 保存连通性检查配置
#[tauri::command]
pub fn cc_switch_save_stream_check_config(
    state: State<'_, AppState>,
...
fn is_copilot_provider(provider: &crate::cc_switch::provider::Provider) -> bool {
    provider
        .meta
        .as_ref()
