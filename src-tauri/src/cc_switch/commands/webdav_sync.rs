...
use crate::cc_switch::commands::sync_support::{
    attach_warning, post_sync_warning_from_result, run_post_import_sync,
};
use crate::cc_switch::error::AppError;
use crate::cc_switch::services::webdav_sync as webdav_sync_service;
use crate::cc_switch::settings::{self, WebDavSyncSettings};
...
#[tauri::command]
pub async fn cc_switch_webdav_test_connection(
...
#[tauri::command]
pub async fn cc_switch_webdav_sync_upload(state: State<'_, AppState>) -> Result<Value, String> {
...
#[tauri::command]
pub async fn cc_switch_webdav_sync_download(state: State<'_, AppState>) -> Result<Value, String> {
    let db = state.db.clone();
    let db_for_sync = db.clone();
    let mut settings = require_enabled_webdav_settings()?;
    let _auto_sync_suppression = crate::cc_switch::services::webdav_auto_sync::AutoSyncSuppressionGuard::new();
...
#[tauri::command]
pub async fn cc_switch_webdav_sync_save_settings(
    settings: WebDavSyncSettings,
...
#[tauri::command]
pub async fn cc_switch_webdav_sync_fetch_remote_info() -> Result<Value, String> {
...
mod tests {
    ...
    use crate::cc_switch::error::AppError;
    use crate::cc_switch::settings::{AppSettings, WebDavSyncSettings};
    ...
    fn persist_sync_error_updates_status_without_overwriting_credentials() {
        ...
        crate::cc_switch::settings::update_settings(AppSettings::default()).expect("reset settings");
        ...
        crate::cc_switch::settings::set_webdav_sync_settings(Some(current.clone()))
            .expect("seed webdav settings");

        persist_sync_error(
            &mut current,
            &crate::cc_switch::error::AppError::Config("boom".to_string()),
            "manual",
        );

        let after = crate::cc_switch::settings::get_webdav_sync_settings().expect("read webdav settings");
    ...
    fn require_enabled_webdav_settings_rejects_disabled_config() {
        ...
        crate::cc_switch::settings::update_settings(AppSettings::default()).expect("reset settings");
        crate::cc_switch::settings::set_webdav_sync_settings(Some(WebDavSyncSettings {
            enabled: false,
            base_url: "https://dav.example.com/dav/".to_string(),
            username: "alice".to_string(),
            password: "secret".to_string(),
