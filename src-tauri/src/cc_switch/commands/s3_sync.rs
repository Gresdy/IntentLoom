...
use crate::cc_switch::services::s3_sync as s3_sync_service;
use crate::cc_switch::settings::{self, S3SyncSettings};
...
#[tauri::command]
pub async fn cc_switch_s3_test_connection(
...
#[tauri::command]
pub async fn cc_switch_s3_sync_upload(state: State<'_, AppState>) -> Result<Value, String> {
...
#[tauri::command]
pub async fn cc_switch_s3_sync_download(state: State<'_, AppState>) -> Result<Value, String> {
    let db = state.db.clone();
    let db_for_sync = db.clone();
    let mut settings = require_enabled_s3_settings()?;
    let _auto_sync_suppression = crate::cc_switch::services::s3_auto_sync::AutoSyncSuppressionGuard::new();
...
#[tauri::command]
pub async fn cc_switch_s3_sync_save_settings(
    settings: S3SyncSettings,
...
#[tauri::command]
pub async fn cc_switch_s3_sync_fetch_remote_info() -> Result<Value, String> {
...
mod tests {
    ...
    use crate::cc_switch::error::AppError;
    use crate::cc_switch::settings::{AppSettings, S3SyncSettings};
    ...
    fn persist_sync_error_updates_status_without_overwriting_credentials() {
        ...
        crate::cc_switch::settings::set_s3_sync_settings(Some(current.clone())).expect("seed s3 settings");
        ...
        let after = crate::cc_switch::settings::get_s3_sync_settings().expect("read s3 settings");
    ...
    fn require_enabled_s3_settings_rejects_disabled_config() {
        ...
        crate::cc_switch::settings::update_settings(AppSettings::default()).expect("reset settings");
        crate::cc_switch::settings::set_s3_sync_settings(Some(S3SyncSettings {
            enabled: false,
            region: "us-east-1".to_string(),
            bucket: "my-bucket".to_string(),
            access_key_id: "AKID".to_string(),
