...
fn merge_settings_for_save(
    mut incoming: crate::cc_switch::settings::AppSettings,
    existing: &crate::cc_switch::settings::AppSettings,
) -> crate::cc_switch::settings::AppSettings {
...
/// 获取设置
#[tauri::command]
pub async fn cc_switch_get_settings() -> Result<crate::cc_switch::settings::AppSettings, String> {
    Ok(crate::cc_switch::settings::get_settings_for_frontend())
}

/// 保存设置
#[tauri::command]
pub async fn cc_switch_save_settings(
    state: tauri::State<'_, crate::cc_switch::store::AppState>,
    settings: crate::cc_switch::settings::AppSettings,
) -> Result<bool, String> {
    let existing = crate::cc_switch::settings::get_settings();
    let merged = merge_settings_for_save(settings, &existing);
    let unify_codex_changed =
        merged.unify_codex_session_history != existing.unify_codex_session_history;
    let unify_codex_enabled = merged.unify_codex_session_history;
    crate::cc_switch::settings::update_settings(merged).map_err(|e| e.to_string())?;
    ...
    if unify_codex_changed {
        ...
        // （本开关的保存只携带开关相关字段）。
        if let Err(err) =
            crate::cc_switch::services::provider::reapply_current_codex_official_live(state.inner())
        {
            log::warn!("统一 Codex 会话历史开关变更后重写 live 配置失败，回滚设置: {err}");
            if let Err(rollback_err) = crate::cc_switch::settings::update_settings(existing) {
        ...
        if unify_codex_enabled {
            ...
            tauri::async_runtime::spawn_blocking(|| {
                match crate::cc_switch::codex_history_migration::maybe_migrate_codex_official_history_to_unified_bucket() {
                    Ok(outcome) => {
            ...
            if let Err(err) = crate::cc_switch::settings::clear_codex_official_history_unify_migration() {
                log::warn!("清除统一会话迁移标记失败: {err}");
            }
            if let Err(err) = crate::cc_switch::settings::clear_codex_unify_migrate_existing() {
...
#[tauri::command]
pub async fn cc_switch_has_codex_unify_history_backup() -> Result<bool, String> {
    Ok(crate::cc_switch::codex_history_migration::has_codex_official_history_unify_backup())
}

/// 按迁移备份账本把当时迁入共享桶的官方会话还原回 "openai" 桶。
/// 由关闭统一会话开关的确认弹窗触发；幂等，可安全重试。
#[tauri::command]
pub async fn cc_switch_restore_codex_unified_history() -> Result<CodexUnifyHistoryRestoreResult, String> {
    let outcome = tauri::async_runtime::spawn_blocking(|| {
        crate::cc_switch::codex_history_migration::restore_codex_official_history_from_backups()
    })
    .await
...
#[tauri::command]
pub async fn cc_switch_restart_app(app: AppHandle) -> Result<bool, String> {
    crate::cc_switch::save_window_state_before_exit(&app);
    ...
    tauri::async_runtime::spawn(async move {
        ...
        crate::cc_switch::cleanup_before_exit(&app).await;
        app.restart();
    });
    Ok(true)
}
...
#[tauri::command]
pub async fn cc_switch_install_update_and_restart(app: AppHandle) -> Result<bool, String> {
    let updater = app
        .updater_builder()
        .build()
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        crate::cc_switch::save_window_state_before_exit(&app);
        crate::cc_switch::cleanup_before_exit(&app).await;
        crate::cc_switch::remove_tray_icon_before_exit(&app);
        crate::cc_switch::destroy_single_instance_lock(&app);
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        // Pre-existing Tauri updater API: `update.install` returns ()
        // in this version of tauri-plugin-updater; swallow the error
        // and return early on failure.
        if let Err(e) = update.install(bytes) {
        ...
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
        let _ = crate::cc_switch::restart_process();
        Ok(true)
    }
}
...
/// 升级无法解决，而不是让其反复尝试。
#[tauri::command]
pub async fn cc_switch_check_app_update_available(app: AppHandle) -> Result<Option<String>, String> {
    let updater = app
        .updater_builder()
        .build()
        .map_err(|e| format!("初始化更新器失败: {e}"))?;
...
#[tauri::command]
pub async fn cc_switch_get_app_config_dir_override(app: AppHandle) -> Result<Option<String>, String> {
    Ok(crate::cc_switch::app_store::refresh_app_config_dir_override(&app)
        .map(|p| p.to_string_lossy().to_string()))
}

/// 设置 app_config_dir 覆盖配置 (到 Store)
#[tauri::command]
pub async fn cc_switch_set_app_config_dir_override(
    app: AppHandle,
    path: Option<String>,
) -> Result<bool, String> {
    crate::cc_switch::app_store::set_app_config_dir_to_store(&app, path.as_deref())?;
    Ok(true)
}

/// 设置开机自启
#[tauri::command]
pub async fn cc_switch_set_auto_launch(enabled: bool) -> Result<bool, String> {
    if enabled {
        crate::cc_switch::auto_launch::enable_auto_launch().map_err(|e| format!("启用开机自启失败: {e}"))?;
    } else {
        crate::cc_switch::auto_launch::disable_auto_launch().map_err(|e| format!("禁用开机自启失败: {e}"))?;
    }
    Ok(true)
}
...
/// 获取开机自启状态
#[tauri::command]
pub async fn cc_switch_get_auto_launch_status() -> Result<bool, String> {
    crate::cc_switch::auto_launch::is_auto_launch_enabled().map_err(|e| format!("获取开机自启状态失败: {e}"))
}

/// 获取整流器配置
#[tauri::command]
pub async fn cc_switch_get_rectifier_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
) -> Result<crate::cc_switch::proxy::types::RectifierConfig, String> {
    state.db.get_rectifier_config().map_err(|e| e.to_string())
}

/// 设置整流器配置
#[tauri::command]
pub async fn cc_switch_set_rectifier_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
    config: crate::cc_switch::proxy::types::RectifierConfig,
) -> Result<bool, String> {
    state
        .db
...
/// 获取优化器配置
#[tauri::command]
pub async fn cc_switch_get_optimizer_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
) -> Result<crate::cc_switch::proxy::types::OptimizerConfig, String> {
    state.db.get_optimizer_config().map_err(|e| e.to_string())
}

/// 设置优化器配置
#[tauri::command]
pub async fn cc_switch_set_optimizer_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
    config: crate::cc_switch::proxy::types::OptimizerConfig,
) -> Result<bool, String> {
...
/// 获取 Copilot 优化器配置
#[tauri::command]
pub async fn cc_switch_get_copilot_optimizer_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
) -> Result<crate::cc_switch::proxy::types::CopilotOptimizerConfig, String> {
    state
        .db
...
/// 设置 Copilot 优化器配置
#[tauri::command]
pub async fn cc_switch_set_copilot_optimizer_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
    config: crate::cc_switch::proxy::types::CopilotOptimizerConfig,
) -> Result<bool, String> {
    state
        .db
...
/// 获取日志配置
#[tauri::command]
pub async fn cc_switch_get_log_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
) -> Result<crate::cc_switch::proxy::types::LogConfig, String> {
    state.db.get_log_config().map_err(|e| e.to_string())
}

/// 设置日志配置
#[tauri::command]
pub async fn cc_switch_set_log_config(
    state: tauri::State<'_, crate::cc_switch::AppState>,
    config: crate::cc_switch::proxy::types::LogConfig,
) -> Result<bool, String> {
    state
        .db
...
/// Stub: cc-switch 在 settings/usage 里调用的用量查询桩。
/// 实际的 cc-switch_queryProviderUsage 在 misc.rs 中注册。
#[allow(dead_code)]
pub fn queryProviderUsage(_provider_id: String, _app: String) -> Result<String, String> { Ok(String::new()) }
