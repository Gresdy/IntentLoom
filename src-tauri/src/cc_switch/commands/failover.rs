...
use crate::cc_switch::database::FailoverQueueItem;
...
/// 获取故障转移队列
#[tauri::command]
pub async fn cc_switch_get_failover_queue(
...
#[tauri::command]
pub async fn cc_switch_get_available_providers_for_failover(
    state: tauri::State<'_, AppState>,
    app_type: String,
...
/// 添加供应商到故障转移队列
#[tauri::command]
pub async fn cc_switch_add_to_failover_queue(
    state: tauri::State<'_, AppState>,
    app_type: String,
    provider_id: String,
) -> Result<(), String> {
...
/// 从故障转移队列移除供应商
#[tauri::command]
pub async fn cc_switch_remove_from_failover_queue(
    state: tauri::State<'_, AppState>,
    app_type: String,
    provider_id: String,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn cc_switch_get_auto_failover_enabled(
    state: tauri::State<'_, AppState>,
    app_type: String,
) -> Result<bool, String> {
    state
...
#[tauri::command]
pub async fn cc_switch_set_auto_failover_enabled(
    app: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    app_type: String,
    enabled: bool,
    ...
    let p1_provider_id = if enabled {
        ...
        if queue.is_empty() {
            let app_enum = crate::cc_switch::app_config::AppType::from_str(&app_type)
                .map_err(|_| format!("无效的应用类型: {app_type}"))?;

            let current_id = crate::cc_switch::settings::get_effective_current_provider(&state.db, &app_enum)
                .map_err(|e| e.to_string())?;
    ...
    // 刷新托盘菜单，确保状态同步
    if let Ok(new_menu) = crate::cc_switch::tray::create_tray_menu(&app, &state) {
