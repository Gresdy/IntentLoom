...
use crate::cc_switch::config::{atomic_write, write_json_file};
use crate::cc_switch::error::AppError;
use crate::cc_switch::opencode_config::get_opencode_dir;
...
impl OmoService {
    ...
    fn write_profile_config(
        ...
        if let Err(err) = crate::cc_switch::opencode_config::add_plugin(v.plugin_name) {
    ...
    pub fn delete_config_file(v: &OmoVariant) -> Result<(), AppError> {
        ...
        crate::cc_switch::opencode_config::remove_plugins_by_prefixes(v.plugin_prefixes)?;
        Ok(())
    }
    ...
    pub fn import_from_local(
        state: &AppState,
        v: &OmoVariant,
    ) -> Result<crate::cc_switch::provider::Provider, AppError> {
        ...
        let provider = crate::cc_switch::provider::Provider {
            id: provider_id,
            name,
            settings_config,
            website_url: None,
