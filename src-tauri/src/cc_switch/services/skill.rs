...
impl SkillService {
    ...
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::cc_switch::settings::get_skill_storage_location();
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        match app {
            ...
            AppType::OpenClaw => {
                if let Some(custom) = crate::cc_switch::settings::get_openclaw_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::Hermes => {
                if let Some(custom) = crate::cc_switch::settings::get_hermes_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
        }
        ...
        Ok(match app {
            ...
            AppType::Hermes => crate::cc_switch::hermes_config::get_hermes_dir().join("skills"),
        })
    }
    ...
    pub fn migrate_storage(
        ...
        let current = crate::cc_switch::settings::get_skill_storage_location();
        if current == target {
        ...
        // 3. 文件移动完成后才持久化设置
        crate::cc_switch::settings::set_skill_storage_location(target)?;
