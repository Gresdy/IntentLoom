...
use crate::cc_switch::config::{get_claude_settings_path, read_json_file, write_json_file};
use crate::cc_switch::database::Database;
use crate::cc_switch::provider::Provider;
use crate::cc_switch::proxy::server::ProxyServer;
use crate::cc_switch::proxy::switch_lock::SwitchLockManager;
use crate::cc_switch::proxy::types::*;
use crate::cc_switch::services::provider::{
...
impl ProxyService {
    ...
    fn has_claude_one_m_marker(model: &str) -> bool {
        model
            .trim_end()
            .to_ascii_lowercase()
            .ends_with(crate::cc_switch::claude_desktop_config::ONE_M_CONTEXT_MARKER)
    }

    fn strip_claude_one_m_marker(model: &str) -> String {
        crate::cc_switch::proxy::model_mapper::strip_one_m_suffix_for_upstream(model)
            .trim()
            .to_string()
    }
    ...
    fn get_current_provider_for_app(&self, app_type: &AppType) -> Result<Option<Provider>, String> {
        let Some(current_id) = crate::cc_switch::settings::get_effective_current_provider(&self.db, app_type)
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        ...
        if enabled {
            ...
            if let Ok(Some(current_id)) =
                crate::cc_switch::settings::get_effective_current_provider(&self.db, &app)
            {
    ...
    async fn sync_live_config_to_provider(
        ...
        match app_type {
            AppType::Claude => {
                let provider_id =
                    crate::cc_switch::settings::get_effective_current_provider(&self.db, &AppType::Claude)
            ...
            AppType::Codex => {
                let provider_id =
                    crate::cc_switch::settings::get_effective_current_provider(&self.db, &AppType::Codex)
            ...
            AppType::Gemini => {
                let provider_id =
                    crate::cc_switch::settings::get_effective_current_provider(&self.db, &AppType::Gemini)
    ...
    fn restore_live_from_ssot_for_app(&self, app_type: &AppType) -> Result<bool, String> {
        let current_id = crate::cc_switch::settings::get_effective_current_provider(&self.db, app_type)
            .map_err(|e| format!("获取 {app_type:?} 当前供应商失败: {e}"))?;
    ...
    fn cleanup_codex_takeover_placeholders_in_live(&self) -> Result<(), String> {
        ...
        if let Some(cfg_str) = config.get("config").and_then(|v| v.as_str()) {
            ...
            let updated =
                crate::cc_switch::codex_config::remove_codex_experimental_bearer_token_if(&updated, |token| {
    ...
    fn remove_local_toml_base_url(toml_str: &str) -> String {
        crate::cc_switch::codex_config::remove_codex_toml_base_url_if(toml_str, Self::is_local_proxy_url)
    }
    ...
    fn codex_live_has_proxy_placeholder(config: &Value) -> bool {
        ...
        config
            .get("config")
            .and_then(|v| v.as_str())
            .and_then(crate::cc_switch::codex_config::extract_codex_experimental_bearer_token)
            .as_deref()
            == Some(PROXY_TOKEN_PLACEHOLDER)
    }
    ...
    async fn update_live_backup_from_provider_inner(
        ...
        if matches!(app_type_enum, AppType::Codex) {
            ...
            crate::cc_switch::codex_config::apply_codex_unified_session_bucket_to_settings(
    ...
    pub(crate) async fn hot_switch_provider_inner(
        ...
        let logical_target_changed =
            crate::cc_switch::settings::get_effective_current_provider(&self.db, &app_type_enum)
        ...
        crate::cc_switch::settings::set_current_provider(&app_type_enum, Some(provider_id))
        ...
        if has_backup && !live_taken_over && matches!(app_type_enum, AppType::Codex) {
            ...
            let profile = crate::cc_switch::codex_config::CodexCatalogToolProfile::from_api_format(
                provider.meta.as_ref().and_then(|m| m.api_format.as_deref()),
            );

            crate::cc_switch::codex_config::write_codex_provider_live_with_catalog(
                &effective_settings,
                provider.category.as_deref(),
                auth,
                config_str,
    ...
    fn preserve_codex_oauth_auth_in_backup(
        ...
        if !crate::cc_switch::settings::preserve_codex_official_auth_on_switch() {
            return Ok(());
        }

        let Some(existing_auth) = existing_backup
            .get("auth")
            .filter(|auth| crate::cc_switch::codex_config::codex_auth_has_oauth_login_material(auth))
            .cloned()
        else {
            return Ok(());
        };
        ...
        if let Some(config_text) = target_obj.get("config").and_then(|value| value.as_str()) {
            let live_config = crate::cc_switch::codex_config::prepare_codex_provider_live_config(
                &provider_auth,
                config_text,
            )
    ...
    fn update_toml_base_url(toml_str: &str, new_url: &str) -> String {
        crate::cc_switch::codex_config::update_codex_toml_field(toml_str, "base_url", new_url)
    ...
    fn apply_codex_proxy_toml_config_for_provider(
        ...
        let mut updated =
            crate::cc_switch::codex_config::update_codex_toml_field(&updated, "wire_api", "responses")
                .unwrap_or(updated);

        if let Some(upstream_model) =
            provider.and_then(crate::cc_switch::proxy::providers::codex_provider_upstream_model)
        {
            updated =
                crate::cc_switch::codex_config::update_codex_toml_field(&updated, "model", &upstream_model)
                    .unwrap_or(updated);
        }
    ...
    fn write_claude_live(&self, config: &Value) -> Result<(), String> {
        let path = get_claude_settings_path();
        let settings = crate::cc_switch::services::provider::sanitize_claude_settings_for_live(config);
        write_json_file(&path, &settings).map_err(|e| format!("写入 Claude 配置失败: {e}"))
    }

    fn read_codex_live(&self) -> Result<Value, String> {
        crate::cc_switch::codex_config::read_codex_live_settings()
            ...
            if crate::cc_switch::settings::preserve_codex_official_auth_on_switch() {
                if let (Some(auth), Some(config_str)) = (
                        ...
                        == Some(PROXY_TOKEN_PLACEHOLDER)
                    {
                        ...
                        .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                        crate::cc_switch::codex_config::write_codex_live_config_atomic(Some(&live_config))
                            .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                        return Ok(());
                    }
                }
    ...
    fn write_codex_takeover_live_for_provider(
        ...
        if crate::cc_switch::settings::preserve_codex_official_auth_on_switch() {
                ...
                .filter(|auth| Self::codex_auth_has_proxy_placeholder(auth))
            {
                ...
                let prepared_config =
                    crate::cc_switch::codex_config::prepare_codex_live_config_text_with_optional_catalog(
                        config, config_str, profile,
                    )
                    .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                let live_config =
                    crate::cc_switch::codex_config::prepare_codex_provider_live_config(auth, &prepared_config)
                        .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                crate::cc_switch::codex_config::write_codex_live_config_atomic(Some(&live_config))
                    .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                return Ok(());
    ...
    fn write_codex_live_verbatim(&self, config: &Value) -> Result<(), String> {
        use crate::cc_switch::codex_config::{get_codex_auth_path, get_codex_config_path};
        ...
        let prepared_cfg = config_str
            .map(|cfg| {
                crate::cc_switch::codex_config::prepare_codex_live_config_text_with_optional_catalog(
                    config,
                    cfg,
                    crate::cc_switch::codex_config::CodexCatalogToolProfile::ProxyChat,
                )
            })
            .transpose()
            .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
        ...
        match (auth, prepared_cfg.as_deref()) {
            (Some(auth), Some(cfg)) => {
                ...
                if auth.as_object().is_some_and(|obj| obj.is_empty()) {
                    let _ = crate::cc_switch::config::delete_file(&auth_path);
                    let config_path = get_codex_config_path();
                    crate::cc_switch::config::write_text_file(&config_path, cfg)
                        .map_err(|e| format!("写入 Codex config 失败: {e}"))?;
                } else {
                    crate::cc_switch::codex_config::write_codex_live_atomic(auth, Some(cfg))
                        .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                }
            }
            (Some(auth), None) => {
    ...
    fn read_gemini_live(&self) -> Result<Value, String> {
        use crate::cc_switch::gemini_config::{env_to_json, get_gemini_env_path, read_gemini_env};
    ...
    fn write_gemini_live(&self, config: &Value) -> Result<(), String> {
        use crate::cc_switch::gemini_config::{json_to_env, write_gemini_env_atomic};
    ...
    pub async fn update_circuit_breaker_configs(
        &self,
        config: crate::cc_switch::proxy::CircuitBreakerConfig,
    ) -> Result<(), String> {
        if let Some(server) = self.server.read().await.as_ref() {
...
mod tests {
    use super::*;
    use crate::cc_switch::provider::ProviderMeta;
    ...
    fn seed_codex_model_template() {
        let codex_dir = crate::cc_switch::codex_config::get_codex_config_dir();
    ...
    async fn start_with_takeover_ephemeral_port_writes_actual_live_url() {
        ...
        crate::cc_switch::settings::reload_settings().expect("reload settings");
        ...
        crate::cc_switch::settings::set_current_provider(&AppType::Claude, Some("p1"))
    ...
    fn codex_custom_provider_live_write_preserves_oauth_auth_json() {
        let _home = TempHome::new();
        crate::cc_switch::settings::reload_settings().expect("reload settings");
        crate::cc_switch::settings::update_settings(crate::cc_switch::settings::AppSettings {
        ...
        crate::cc_switch::codex_config::write_codex_live_atomic(
            &oauth_auth,
            Some(
...
"#
        ...
        let live_auth: Value =
            crate::cc_switch::config::read_json_file(&crate::cc_switch::codex_config::get_codex_auth_path())
        ...
        let live_config = std::fs::read_to_string(crate::cc_switch::codex_config::get_codex_config_path())
        ...
        crate::cc_switch::settings::update_settings(crate::cc_switch::settings::AppSettings::default())
...
"#;
        crate::cc_switch::codex_config::write_codex_live_atomic(&oauth_auth, Some(deepseek_live_config))
...
"#
        ...
        crate::cc_switch::settings::set_current_provider(&AppType::Codex, Some("deepseek"))
            .expect("set local current provider");
...
"#
    ...
    async fn codex_sync_current_to_live_during_takeover_preserves_oauth_auth_json() {
        ...
        let db = Arc::new(Database::memory().expect("init db"));
        use_ephemeral_proxy_port(&db).await;
        let state = crate::cc_switch::store::AppState::new(db.clone());
        let oauth_auth = json!({
            "auth_mode": "chatgpt",
            "tokens": {
                "id_token": "oauth-id",
...
"#
        ...
        crate::cc_switch::services::provider::ProviderService::sync_current_to_live(&state)
...
"#;
        crate::cc_switch::codex_config::write_codex_live_atomic(&oauth_auth, Some(stale_live_config))
...
"#;
    ...
    async fn switch_proxy_target_updates_live_backup_when_taken_over() {
        ...
        assert_eq!(
            crate::cc_switch::settings::get_current_provider(&AppType::Claude).as_deref(),
            Some("b")
        );
    ...
    async fn hot_switch_provider_updates_claude_live_while_preserving_takeover_fields() {
        ...
        db.set_current_provider("claude", "a")
            .expect("set current provider");
        crate::cc_switch::settings::set_current_provider(&AppType::Claude, Some("a"))
            .expect("set local current provider");
        db.save_live_backup(
            "claude",
    ...
    async fn hot_switch_provider_serializes_same_app_switches() {
        ...
        assert_eq!(
            crate::cc_switch::settings::get_effective_current_provider(&db, &AppType::Claude)
...
"#
        ...
        db.set_current_provider("codex", "a")
            .expect("set current provider");
        crate::cc_switch::settings::set_current_provider(&AppType::Codex, Some("a"))
            .expect("set local current provider");
        db.save_live_backup(
            "codex",
            &serde_json::to_string(&provider_a.settings_config).expect("serialize provider a"),
...
"#;
        ...
        crate::cc_switch::services::provider::ProviderService::switch(&state, AppType::Codex, "b")
            .expect("provider switch to provider b");
        state.proxy_service.stop().await.expect("stop proxy server");

        let catalog_path = crate::cc_switch::codex_config::get_codex_model_catalog_path();
        assert!(
            catalog_path.exists(),
        ...
        let config_path = crate::cc_switch::codex_config::get_codex_config_path();
...
"#;
        ...
        let err = crate::cc_switch::services::provider::ProviderService::switch(&state, AppType::Codex, "b")
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        let restored = std::fs::read_to_string(crate::cc_switch::codex_config::get_codex_config_path())
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            crate::cc_switch::codex_config::get_codex_model_catalog_path().exists(),
            "empty-auth restore must generate the cc-switch catalog file"
        );
        assert!(
            !crate::cc_switch::codex_config::get_codex_auth_path().exists(),
    ...
    async fn bulk_backup_skips_all_when_live_is_proxy_placeholder() {
        ...
        std::fs::create_dir_all(&codex_dir).expect("create codex dir");
        std::fs::write(
            crate::cc_switch::codex_config::get_codex_config_path(),
...
"#,
        ...
        std::fs::write(
            crate::cc_switch::codex_config::get_codex_auth_path(),
            r#"{"OPENAI_API_KEY":"PROXY_MANAGED"}"#,
        )
        .expect("seed codex auth.json");
        let gemini_env_path = crate::cc_switch::gemini_config::get_gemini_env_path();
