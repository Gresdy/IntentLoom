...
use crate::cc_switch::proxy::usage::calculator::ModelPricing;
use crate::cc_switch::services::sql_helpers::fresh_input_sql;
...
fn clean_model_id_for_pricing(model_id: &str) -> String {
    ...
    normalized
        .trim_end_matches(crate::cc_switch::claude_desktop_config::ONE_M_CONTEXT_MARKER)
        .trim()
        .to_string()
}
...
mod tests {
    ...
    fn test_effective_filter_keeps_legacy_null_data_source_proxy_rows() -> Result<(), AppError> {
        ...
        let count: i64 = conn.query_row(&sql, [], |row: &rusqlite::Row| row.get(0))?;
        assert_eq!(count, 1);
    ...
    fn test_effective_usage_dedup_prefers_proxy_for_session_sources() -> Result<(), AppError> {
        ...
        let breakdown = crate::cc_switch::services::session_usage::get_data_source_breakdown(&db)?;
