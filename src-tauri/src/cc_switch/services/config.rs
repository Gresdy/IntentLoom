...
use crate::cc_switch::app_config::{AppType, MultiAppConfig};
...
impl ConfigService {
    ...
    fn sync_codex_live(
        ...
        let cfg_text_after = crate::cc_switch::codex_config::read_and_validate_codex_config_text()?;
        if let Some(manager) = config.get_manager_mut(&AppType::Codex) {
            if let Some(target) = manager.providers.get_mut(provider_id) {
                if let Some(obj) = target.settings_config.as_object_mut() {
                    ...
                    crate::cc_switch::codex_config::restore_codex_settings_for_backfill(
                        &mut restored,
                        &provider.settings_config,
    ...
    fn sync_claude_live(
        ...
        use crate::cc_switch::config::{read_json_file, write_json_file};

        let settings_path = crate::cc_switch::config::get_claude_settings_path();
    ...
    fn sync_gemini_live(
        ...
        use crate::cc_switch::gemini_config::{env_to_json, read_gemini_env};

        ProviderService::write_gemini_live(provider)?;

        // 读回实际写入的内容并更新到配置中（包含 settings.json）
        let live_after_env = read_gemini_env()?;
        let settings_path = crate::cc_switch::gemini_config::get_gemini_settings_path();
        let live_after_config = if settings_path.exists() {
            crate::cc_switch::config::read_json_file(&settings_path)?
        } else {
            serde_json::json!({})
        };
