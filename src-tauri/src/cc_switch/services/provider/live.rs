...
use crate::cc_switch::config::{delete_file, get_claude_settings_path, read_json_file, write_json_file};
...
pub(crate) fn provider_exists_in_live_config(
    ...
    match app_type {
        AppType::OpenCode => crate::cc_switch::opencode_config::get_providers()
            .map(|providers| providers.contains_key(provider_id)),
        AppType::OpenClaw => crate::cc_switch::openclaw_config::get_providers()
            .map(|providers| providers.contains_key(provider_id)),
        AppType::Hermes => crate::cc_switch::hermes_config::get_providers()
            .map(|providers| providers.contains_key(provider_id)),
        _ => Ok(false),
    }
}
...
pub(crate) fn write_live_with_common_config(
    ...
    if matches!(app_type, AppType::ClaudeDesktop) {
        crate::cc_switch::claude_desktop_config::apply_provider(db, &effective_provider)?;
        log::info!(
            "Claude Desktop 3P profile '{}' written for provider '{}'",
            crate::cc_switch::claude_desktop_config::PROFILE_ID,
...
fn restore_live_settings_for_provider_backfill(
    ...
    let restore_provider_token =
        crate::cc_switch::codex_config::should_restore_codex_provider_token_for_backfill(
            provider.category.as_deref(),
            &provider.settings_config,
        );
    if let Err(err) = crate::cc_switch::codex_config::restore_codex_settings_for_backfill(
        &mut settings,
        &provider.settings_config,
    ...
    if provider.category.as_deref() == Some("official") {
        if let Err(err) =
            crate::cc_switch::codex_config::strip_codex_unified_session_bucket_from_settings(&mut settings)
        {
            log::warn!(
        ...
        match self {
            ...
            LiveSnapshot::Codex { auth, config } => {
                ...
                if let Some(text) = config {
                    crate::cc_switch::config::write_text_file(&config_path, text)?;
...
pub(crate) fn write_live_snapshot(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
    match app_type {
        ...
        AppType::OpenCode => {
            ...
            use crate::cc_switch::opencode_config;
            use crate::cc_switch::provider::OpenCodeProviderConfig;
        ...
        AppType::OpenClaw => {
            ...
            use crate::cc_switch::openclaw_config;
            use crate::cc_switch::openclaw_config::OpenClawProviderConfig;
        ...
        AppType::Hermes => {
            crate::cc_switch::hermes_config::set_provider(&provider.id, provider.settings_config.clone())?;
...
pub(crate) fn sync_current_provider_for_app_to_live(
    ...
    if app_type.is_additive_mode() {
        ...
        let current_id = match crate::cc_switch::settings::get_effective_current_provider(&state.db, app_type)?
        {
            Some(id) => id,
            None => return Ok(()),
        };
...
pub fn sync_current_to_live(state: &AppState) -> Result<(), AppError> {
    ...
    // Skill sync
    for app_type in AppType::all() {
        if let Err(e) = crate::cc_switch::services::skill::SkillService::sync_to_app(&state.db, &app_type) {
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            let mut result = crate::cc_switch::codex_config::read_codex_live_settings()?;
            ...
            if let Ok(Some(model_catalog)) =
                crate::cc_switch::codex_config::read_codex_model_catalog_simplified_from_live()
            {
                if let Some(obj) = result.as_object_mut() {
        ...
        AppType::OpenClaw => {
            use crate::cc_switch::openclaw_config::{get_openclaw_config_path, read_openclaw_config};
        ...
        AppType::Hermes => {
            let config_path = crate::cc_switch::hermes_config::get_hermes_config_path();
            if !config_path.exists() {
                return Err(AppError::localized(
            ...
            let yaml_config = crate::cc_switch::hermes_config::read_hermes_config()?;
            let config = crate::cc_switch::hermes_config::yaml_to_json(&yaml_config)?;
            Ok(config)
        }
    }
}
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    let settings_config = match app_type {
        AppType::Codex => crate::cc_switch::codex_config::read_codex_live_settings()?,
        AppType::Claude => {
    ...
    provider.category = Some(
        if matches!(app_type, AppType::Codex) {
            ...
            let has_provider_key = crate::cc_switch::codex_config::extract_codex_api_key(
            ...
            let has_login_material = provider
                .settings_config
                .get("auth")
                .is_some_and(crate::cc_switch::codex_config::codex_auth_has_login_material);
...
pub(crate) fn write_gemini_live(provider: &Provider) -> Result<(), AppError> {
    ...
    match auth_type {
        ...
        GeminiAuthType::Packycode | GeminiAuthType::Generic => {
            crate::cc_switch::gemini_config::write_packycode_settings()?;
        }
    }
...
pub fn import_opencode_providers_from_live(state: &AppState) -> Result<usize, AppError> {
    ...
    for (id, config) in providers {
        ...
        provider.meta = Some(crate::cc_switch::provider::ProviderMeta {
...
pub fn import_hermes_providers_from_live(state: &AppState) -> Result<usize, AppError> {
    use crate::cc_switch::hermes_config;
