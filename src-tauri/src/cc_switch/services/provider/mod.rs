...
use crate::cc_switch::database::{validate_cost_multiplier, validate_pricing_source};
use crate::cc_switch::error::AppError;
use crate::cc_switch::provider::{Provider, UsageResult};
use crate::cc_switch::services::mcp::McpService;
use crate::cc_switch::settings::CustomEndpoint;
...
mod tests {
    ...
    use crate::cc_switch::claude_desktop_config::PROFILE_ID;
    use crate::cc_switch::config::{get_claude_settings_path, read_json_file, write_json_file};
    use crate::cc_switch::database::Database;
    #[cfg(any(target_os = "macos", windows))]
    use crate::cc_switch::provider::{ClaudeDesktopMode, ClaudeDesktopModelRoute};
    use crate::cc_switch::provider::{ProviderMeta, UsageScript};
    use crate::cc_switch::proxy::types::ProxyConfig;
    use crate::cc_switch::store::AppState;
    use serde_json::json;
    ...
    fn omo_config_path(home: &Path, category: &str) -> PathBuf {
        home.join(".config").join("opencode").join(match category {
            "omo" => crate::cc_switch::services::omo::STANDARD.preferred_filename,
            "omo-slim" => crate::cc_switch::services::omo::SLIM.preferred_filename,
...
"#;
    ...
    #[tokio::test]
    #[serial]
    #[ignore = "flaky: hardcoded 127.0.0.1:15721 port collides with previous test runs (os error 48)"]
    async fn update_current_claude_desktop_provider_syncs_profile_when_proxy_takeover_is_active() {
        ...
        db.set_current_provider("claude-desktop", "p1")
            .expect("set current provider");
        crate::cc_switch::settings::set_current_provider(&AppType::ClaudeDesktop, Some("p1"))
            .expect("set local current provider");
    ...
    fn sync_current_provider_for_app_skips_db_only_opencode_provider() {
        with_test_home(|state, _| {
            ...
            let live_providers = crate::cc_switch::opencode_config::get_providers()
    ...
    fn sync_current_provider_for_app_skips_db_only_openclaw_provider() {
        with_test_home(|state, _| {
            ...
            let live_providers = crate::cc_switch::openclaw_config::get_providers()
    ...
    fn sync_current_provider_for_app_preserves_legacy_live_opencode_provider() {
        with_test_home(|state, _| {
            ...
            crate::cc_switch::opencode_config::set_provider(&provider.id, provider.settings_config.clone())
            ...
            let live_providers =
                crate::cc_switch::opencode_config::get_providers().expect("read opencode providers");
            assert_eq!(
                live_providers
                    .get(&provider.id)
    ...
    fn sync_current_provider_for_app_restores_legacy_openclaw_provider_after_live_reset() {
        with_test_home(|state, _| {
            ...
            let live_providers =
                crate::cc_switch::openclaw_config::get_providers().expect("read openclaw providers");
            assert!(
                live_providers.contains_key(&provider.id),
    ...
    fn import_openclaw_providers_from_live_marks_provider_as_live_managed() {
        with_test_home(|state, _| {
            ...
            crate::cc_switch::openclaw_config::set_provider(&provider.id, provider.settings_config.clone())
...
impl ProviderService {
    ...
    pub fn current(state: &AppState, app_type: AppType) -> Result<String, AppError> {
        ...
        crate::cc_switch::settings::get_effective_current_provider(&state.db, &app_type)
    ...
    pub fn update(
        ...
        if provider_id_changed {
            ...
            if crate::cc_switch::settings::get_current_provider(&app_type).as_deref() == Some(&original_id) {
                crate::cc_switch::settings::set_current_provider(&app_type, Some(provider.id.as_str()))?;
            }
        ...
        if app_type.is_additive_mode() {
            let omo_variant = if matches!(app_type, AppType::OpenCode) {
                match provider.category.as_deref() {
                    Some("omo") => Some(&crate::cc_switch::services::omo::STANDARD),
                    Some("omo-slim") => Some(&crate::cc_switch::services::omo::SLIM),
                    _ => None,
                }
            } else {
                None
            ...
            if let Some(variant) = omo_variant {
                ...
                if is_current {
                    crate::cc_switch::services::OmoService::write_provider_config_to_file(&provider, variant)?;
                }
                if let Err(err) = state.db.save_provider(app_type.as_str(), &provider) {
                    if is_current {
                        if let Err(rollback_err) =
                            crate::cc_switch::services::OmoService::write_config_to_file(state, variant)
                        {
                            log::warn!(
    ...
    pub fn delete(state: &AppState, app_type: AppType, id: &str) -> Result<(), AppError> {
        ...
        if app_type.is_additive_mode() {
            ...
            if matches!(app_type, AppType::OpenCode) {
                ...
                if let Some(variant) = omo_variant {
                    ...
                    if was_current {
                        crate::cc_switch::services::OmoService::delete_config_file(variant)?;
                    }
                    return Ok(());
                }
            }
        ...
        let local_current = crate::cc_switch::settings::get_current_provider(&app_type);
    ...
    fn switch_normal(
        ...
        if matches!(app_type, AppType::OpenCode) {
            let omo_pair = match provider.category.as_deref() {
                Some("omo") => Some((&crate::cc_switch::services::omo::STANDARD, &crate::cc_switch::services::omo::SLIM)),
                Some("omo-slim") => {
                    Some((&crate::cc_switch::services::omo::SLIM, &crate::cc_switch::services::omo::STANDARD))
                }
                _ => None,
            };
            if let Some((enable, disable)) = omo_pair {
                ...
                crate::cc_switch::services::OmoService::write_config_to_file(state, enable)?;
                let _ = crate::cc_switch::services::OmoService::delete_config_file(disable);
                return Ok(SwitchResult::default());
            }
        }
        ...
        let current_id = crate::cc_switch::settings::get_effective_current_provider(&state.db, &app_type)?;
        ...
        if !app_type.is_additive_mode() {
            ...
            crate::cc_switch::settings::set_current_provider(&app_type, Some(id))?;
        ...
        if matches!(app_type, AppType::Hermes) {
            if let Err(e) =
                crate::cc_switch::hermes_config::apply_switch_defaults(&provider.id, &provider.settings_config)
            {
                log::warn!(
    ...
    pub fn sync_current_provider_for_app(
        ...
        let current_id =
            match crate::cc_switch::settings::get_effective_current_provider(&state.db, &app_type)? {
                Some(id) => id,
                None => return Ok(()),
            };
    ...
    fn validate_provider_settings(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
        match app_type {
            ...
            AppType::ClaudeDesktop => {
                crate::cc_switch::claude_desktop_config::validate_provider(provider)?;
            }
            AppType::Codex => {
                ...
                if let Some(config_value) = settings.get("config") {
                    ...
                    if let Some(cfg_text) = config_value.as_str() {
                        crate::cc_switch::codex_config::validate_config_toml(cfg_text)?;
                    }
                }
            }
            AppType::Gemini => {
                use crate::cc_switch::gemini_config::validate_gemini_settings;
    ...
    fn extract_credentials(
        ...
        match app_type {
            ...
            AppType::ClaudeDesktop => {
                let credentials =
                    crate::cc_switch::claude_desktop_config::direct_gateway_credentials(provider)?;
            ...
            AppType::Codex => {
                ...
                let api_key = crate::cc_switch::codex_config::extract_codex_api_key(
            ...
            AppType::Gemini => {
                use crate::cc_switch::gemini_config::json_to_env;
