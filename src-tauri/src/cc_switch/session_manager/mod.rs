...
fn provider_roots(provider_id: &str) -> Result<Vec<PathBuf>, String> {
    let roots = match provider_id {
        ...
        "claude" => vec![crate::cc_switch::config::get_claude_config_dir().join("projects")],
        "opencode" => vec![opencode::get_opencode_data_dir()],
        "openclaw" => vec![crate::cc_switch::openclaw_config::get_openclaw_dir().join("agents")],
        "gemini" => vec![crate::cc_switch::gemini_config::get_gemini_dir().join("tmp")],
        "hermes" => vec![crate::cc_switch::hermes_config::get_hermes_dir().join("sessions")],
