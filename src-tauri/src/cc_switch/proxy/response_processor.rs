...
///
/// `body_timeout`: 整包超时。当非零时用 `tokio::time::timeout` 包住 `.bytes_stream()` 调用，
            ...
            match chunk_result {
                Some(Ok(bytes)) => {
                    ...
                    if inspect_sse_events {
                        crate::cc_switch::proxy::sse::append_utf8_safe(&mut buffer, &mut utf8_remainder, &bytes);
...
mod tests {
    ...
    use crate::cc_switch::proxy::failover_switch::FailoverSwitchManager;
    use crate::cc_switch::proxy::provider_router::ProviderRouter;
    use crate::cc_switch::proxy::providers::{
        codex_chat_history::CodexChatHistoryStore, gemini_shadow::GeminiShadowStore,
    };
    use crate::cc_switch::proxy::types::{ProxyConfig, ProxyStatus};
    ...
    async fn test_claude_desktop_inherits_claude_global_defaults() -> Result<(), AppError> {
        use crate::cc_switch::proxy::usage::logger::UsageLogger;
