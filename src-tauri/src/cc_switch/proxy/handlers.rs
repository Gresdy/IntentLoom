...
use crate::cc_switch::database::PRICING_SOURCE_REQUEST;
...
pub async fn handle_models() -> Result<Json<Value>, ProxyError> {
    let generated_path = crate::cc_switch::codex_config::get_codex_model_catalog_path();
    let active_catalog_path = match crate::cc_switch::codex_config::read_codex_config_text() {
        Ok(config_text) => {
            crate::cc_switch::codex_config::resolve_cc_switch_catalog_path(&config_text, &generated_path)
        }
        Err(_) => None,
    };
...
pub async fn handle_claude_desktop_models(
    ...
    let response = crate::cc_switch::claude_desktop_config::model_list_response(provider)
...
fn validate_claude_desktop_gateway_auth(
    ...
    let expected = crate::cc_switch::claude_desktop_config::get_or_create_gateway_token(state.db.as_ref())
...
mod tests {
    ...
    use crate::cc_switch::proxy::ProxyError;
