...
use crate::cc_switch::database::{Database, PRICING_SOURCE_REQUEST, PRICING_SOURCE_RESPONSE};
use crate::cc_switch::error::AppError;
use crate::cc_switch::services::usage_stats::{find_model_pricing_row, is_placeholder_pricing_model};
...
impl<'a> UsageLogger<'a> {
    ...
    pub fn log_request(&self, log: &RequestLog) -> Result<(), AppError> {
        let conn = crate::cc_switch::database::lock_conn!(self.db.conn);
