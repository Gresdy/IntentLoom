...
use crate::cc_switch::config::get_home_dir;
use crate::cc_switch::config::{atomic_write, delete_file, read_json_file, write_json_file};
use crate::cc_switch::database::Database;
use crate::cc_switch::database::CLAUDE_DESKTOP_OFFICIAL_PROVIDER_ID;
...
pub fn get_status(db: &Database, proxy_running: bool) -> Result<ClaudeDesktopStatus, AppError> {
    ...
    let current_provider = crate::cc_switch::settings::get_effective_current_provider(
        db,
        &crate::cc_switch::app_config::AppType::ClaudeDesktop,
    )
    .ok()
    .flatten()
...
mod tests {
    use super::*;
    use crate::cc_switch::database::Database;
    use crate::cc_switch::provider::{ClaudeDesktopModelRoute, ProviderMeta};
    use serde_json::json;
    ...
    fn set_proxy_port(db: &Database, port: u16) {
        let config = crate::cc_switch::proxy::types::ProxyConfig {
            listen_port: port,
            ..Default::default()
        };
