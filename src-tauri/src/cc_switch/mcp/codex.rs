...
use crate::cc_switch::app_config::{McpApps, McpConfig, McpServer, MultiAppConfig};
...
fn should_sync_codex_mcp() -> bool {
    ...
    crate::cc_switch::codex_config::get_codex_config_dir().exists()
}
...
pub fn import_from_codex(config: &mut MultiAppConfig) -> Result<usize, AppError> {
    let text = crate::cc_switch::codex_config::read_and_validate_codex_config_text()?;
...
pub fn sync_enabled_to_codex(config: &MultiAppConfig) -> Result<(), AppError> {
    ...
    let base_text = crate::cc_switch::codex_config::read_and_validate_codex_config_text()?;
    ...
    let path = crate::cc_switch::codex_config::get_codex_config_path();
    crate::cc_switch::config::write_text_file(&path, &new_text)?;
    Ok(())
}
...
pub fn sync_single_server_to_codex(
    ...
    // 写回文件
    let new_text = doc.to_string();
    crate::cc_switch::config::write_text_file(&config_path, &new_text)?;
