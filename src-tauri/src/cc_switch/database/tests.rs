...
use super::*;
use crate::cc_switch::app_config::MultiAppConfig;
use crate::cc_switch::provider::{Provider, ProviderManager};
...
fn schema_model_pricing_is_seeded_on_init() {
    ...
    let count: i64 = conn
        .query_row("SELECT COUNT(*) FROM model_pricing", [], |row: &rusqlite::Row| row.get(0))
