...
impl Database {
        ...
        for provider_res in provider_iter {
            ...
            let endpoints_iter = stmt_endpoints
                .query_map(params![id, app_type], |row| {
                    ...
                    Ok((
                        url,
                        crate::cc_switch::settings::CustomEndpoint {
                            url: "".to_string(),
    ...
    pub fn get_current_omo_provider(
        ...
        let meta: crate::cc_switch::provider::ProviderMeta = if meta_str.trim().is_empty() {
            crate::cc_switch::provider::ProviderMeta::default()
        } else {
    ...
    pub fn has_non_official_seed_provider(&self, app_type: &str) -> Result<bool, AppError> {
        use crate::cc_switch::database::dao::providers_seed::is_official_seed_id;
        let conn = lock_conn!(self.conn);
        let mut stmt = conn
            .prepare("SELECT id FROM providers WHERE app_type = ?1")
            .map_err(|e| AppError::Database(e.to_string()))?;
    ...
    pub fn init_default_official_providers(&self) -> Result<usize, AppError> {
        use crate::cc_switch::database::dao::providers_seed::OFFICIAL_SEEDS;
    ...
    pub fn ensure_official_seed_by_id(
        &self,
        seed_id: &str,
        app_type: crate::cc_switch::app_config::AppType,
    ) -> Result<bool, AppError> {
        use crate::cc_switch::database::dao::providers_seed::OFFICIAL_SEEDS;
...
mod ensure_official_seed_tests {
    ...
    use crate::cc_switch::database::{Database, CLAUDE_DESKTOP_OFFICIAL_PROVIDER_ID};
