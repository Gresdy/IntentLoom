...
impl Database {
    ...
    pub fn get_rectifier_config(&self) -> Result<crate::cc_switch::proxy::types::RectifierConfig, AppError> {
        match self.get_setting("rectifier_config")? {
            Some(json) => serde_json::from_str(&json)
                .map_err(|e| AppError::Database(format!("解析整流器配置失败: {e}"))),
            None => Ok(crate::cc_switch::proxy::types::RectifierConfig::default()),
        }
    }
    ...
    pub fn get_optimizer_config(&self) -> Result<crate::cc_switch::proxy::types::OptimizerConfig, AppError> {
        match self.get_setting("optimizer_config")? {
            Some(json) => serde_json::from_str(&json)
                .map_err(|e| AppError::Database(format!("解析优化器配置失败: {e}"))),
            None => Ok(crate::cc_switch::proxy::types::OptimizerConfig::default()),
        }
    }
    ...
    pub fn get_copilot_optimizer_config(
        &self,
    ) -> Result<crate::cc_switch::proxy::types::CopilotOptimizerConfig, AppError> {
        match self.get_setting("copilot_optimizer_config")? {
            Some(json) => serde_json::from_str(&json)
                .map_err(|e| AppError::Database(format!("解析 Copilot 优化器配置失败: {e}"))),
            None => Ok(crate::cc_switch::proxy::types::CopilotOptimizerConfig::default()),
        }
    }
    ...
    /// 获取日志配置
    pub fn get_log_config(&self) -> Result<crate::cc_switch::proxy::types::LogConfig, AppError> {
        match self.get_setting("log_config")? {
            Some(json) => serde_json::from_str(&json)
                .map_err(|e| AppError::Database(format!("解析日志配置失败: {e}"))),
            None => Ok(crate::cc_switch::proxy::types::LogConfig::default()),
        }
    }

    /// 更新日志配置
    pub fn set_log_config(&self, config: &crate::cc_switch::proxy::types::LogConfig) -> Result<(), AppError> {
        let json = serde_json::to_string(config)
