...
impl Database {
    ...
    pub fn export_sql(&self, target_path: &Path) -> Result<(), AppError> {
        ...
        crate::cc_switch::config::atomic_write(target_path, dump.as_bytes())
    }
    ...
    pub(crate) fn periodic_backup_if_needed(&self) -> Result<(), AppError> {
        let interval_hours = crate::cc_switch::settings::effective_backup_interval_hours();
        if interval_hours > 0 {
    ...
    fn cleanup_db_backups(dir: &Path) -> Result<(), AppError> {
        let retain = crate::cc_switch::settings::effective_backup_retain_count();
    ...
    fn validate_basic_state(conn: &Connection) -> Result<(), AppError> {
        let provider_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM providers", [], |row: &rusqlite::Row| row.get(0))
            .map_err(|e| AppError::Database(e.to_string()))?;
        let mcp_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM mcp_servers", [], |row: &rusqlite::Row| row.get(0))
            .map_err(|e| AppError::Database(e.to_string()))?;
    ...
    fn dump_sql(conn: &Connection, skip_tables: &[&str]) -> Result<String, AppError> {
        ...
        let user_version: i64 = conn
            .query_row("PRAGMA user_version;", [], |row: &rusqlite::Row| row.get(0))
            .unwrap_or(0);
...
mod tests {
    use super::Database;
    use crate::cc_switch::error::AppError;
    use crate::cc_switch::settings::{update_settings, AppSettings};
    ...
    fn sync_import_preserves_local_only_tables() -> Result<(), AppError> {
        ...
        {
            let conn = crate::cc_switch::database::lock_conn!(remote_db.conn);
            conn.execute(
        ...
        {
            let conn = crate::cc_switch::database::lock_conn!(local_db.conn);
            conn.execute(
                "INSERT INTO providers (id, app_type, name, settings_config, meta)
