...
use crate::skills::core::skill_store::{ProjectRecord, SkillRecord, SkillStore};
use crate::skills::core::timing::should_log_first_or_slow;
use crate::skills::core::{error::AppError, installer, project_scanner, sync_engine, tool_adapters};
...
pub(crate) fn classify_sync_status(
    ...
    if let Some(project_hash) = skill.content_hash.as_deref() {
        if let Ok(live_center_hash) =
            crate::skills::core::content_hash::hash_directory(Path::new(&managed.central_path))
        {
...
mod tests {
    ...
    use crate::skills::core::content_hash;
    use crate::skills::core::error::ErrorKind;
    use crate::skills::core::project_scanner::ProjectSkillInfo;
    use crate::skills::core::skill_store::SkillRecord;
    use std::fs;
