...
use crate::skills::core::error::AppError;
use crate::skills::core::scenario_service;
use crate::skills::core::scenario_service::sync_scenario_skills;
use crate::skills::core::skill_store::SkillStore;
use crate::skills::core::sync_engine;
use crate::skills::core::timing::should_log_first_or_slow;
use crate::skills::core::tool_adapters::{self, CustomToolDef, ToolCategory};
use crate::skills::core::tool_service::{
...
fn refresh_tray_menu_best_effort(app: &AppHandle) {
    if let Err(err) = crate::skills::tray_stubs::refresh_tray_menu(app) {
...
pub async fn remove_custom_tool(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        for target in targets.iter().filter(|t| t.tool == key) {
            crate::skills::core::sync_engine::remove_target(&PathBuf::from(&target.target_path)).ok();
...
mod tests {
    use super::*;
    use crate::skills::core::skill_store::{ScenarioRecord, SkillRecord};
    use std::fs;
